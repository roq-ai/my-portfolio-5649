...
import { deleteContactFormById } from 'apiSdk/contact-forms';
import { deleteProjectById } from 'apiSdk/projects';
import { deleteSkillById } from 'apiSdk/skills';
...
function OrganizationViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'contact_form', 'project', 'skill'],
      }),
  );

  const contact_formHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
  ...
  const skillHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('contact_form', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Contact Forms
                    </Text>
                    <NextLink passHref href={`/contact-forms/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.contact_form?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/contact-forms/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            <Td>{record.email}</Td>
                            ...
                            <Td>
                              {hasAccess('contact_form', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/contact-forms/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('contact_form', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    contact_formHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('skill', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/skills/create?organization_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.skill?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/skills/view/${record.id}`)} key={record.id}>
                            <Td>{record.name}</Td>
                            <Td>
